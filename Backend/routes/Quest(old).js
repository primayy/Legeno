...
router.post('/checkQuest',function(req,res){
  ...
  connection.query("select * from Billage.quest",function(err,rows,fields){
    if(!err){
      ...
      // }
      //주간 계획 소비 확인
      checkWeeklyPlan(postdata.today,postdata.day,postdata.daily,postdata.expectation).then(function(res){
        if(res==1){
...
async function checkWeeklyPlan(today,day,Data,expectation){
  ...
  var i=1;
  while(i<day){
    var targetDay=await calDate(today,i)
    console.log(targetDay);
    var tDisPlan=await checkDailyPlan(targetDay,Data,expectation)
    if(tDisPlan==1) count++
    i++
  }
  if(count>=7) return 3
...
//   var count=0;
//   for(i=1;i<day;i++){
//     calDate(today,i).then(function(res){
//         // console.log("td:" +res)
//         return checkDailyPlan(res,Data,expectation).then(function(res){
//           if(res==1) {
//             count++
//             if(i==day-1){
//               if(count>=7) return 3
